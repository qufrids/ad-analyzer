...
function ScoreBar({ label, score }: { label: string; score: number }) {
  return (
    <div className="flex items-center gap-2 md:gap-3">
      <span className="w-24 md:w-36 text-[11px] md:text-[12px] text-[#64748B] shrink-0 truncate">{label}</span>
      <div className="flex-1 h-1.5 bg-[#E2E8F0] rounded-full overflow-hidden">
        <div className="h-full rounded-full" style={{ width: `${score}%`, backgroundColor: scoreColor(score) }} />
      </div>
      <span className="w-6 text-right text-[11px] md:text-[12px] font-semibold text-[#0F172A]">{score}</span>
    </div>
  );
}
...
function AnalysisMockup() {
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 md:p-5">
      <div className="flex flex-col md:flex-row gap-4 md:gap-5">
        <div className="shrink-0">
          <div className="w-full md:w-44 h-24 bg-[#F1F5F9] rounded-lg flex items-center justify-center mb-3">
          ...
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-[32px] md:text-[40px] font-black text-[#16A34A] leading-none">94</span>
                <span className="text-[14px] text-[#94A3B8]">/100</span>
              </div>
              <span className="inline-block mt-1 text-[10px] font-semibold text-[#16A34A] bg-[#F0FDF4] border border-[#BBF7D0] px-2 py-0.5 rounded-full">Excellent</span>
            </div>
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 md:p-5">
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="bg-[#FEF2F2] border border-[#FECACA] rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-semibold text-[#EF4444] uppercase tracking-wider">Original</span>
            <span className="text-[20px] font-black text-[#EF4444]">32</span>
          </div>
          {items.map((it) => (
            <div key={it.before} className="text-[11px] text-[#7F1D1D] bg-white/60 rounded px-2 py-1 mb-1.5 line-through opacity-70 truncate">{it.before}</div>
          ))}
        </div>
        <div className="bg-[#F0FDF4] border border-[#BBF7D0] rounded-lg p-3">
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] font-semibold text-[#16A34A] uppercase tracking-wider">Improved</span>
            <span className="text-[20px] font-black text-[#16A34A]">91</span>
          </div>
          {items.map((it) => (
            <div key={it.after} className="text-[11px] text-[#14532D] bg-white/60 rounded px-2 py-1 mb-1.5 truncate">{it.after}</div>
          ))}
        </div>
      </div>
      <div className="flex items-center gap-2 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-3 py-2">
        ...
        <span className="text-[11px] text-[#334155]">Score jumped <strong>+59 points</strong> after AI improvements.</span>
      </div>
    </div>
  );
}
    ...
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 md:p-5">
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="border border-[#E2E8F0] rounded-xl p-3">
          ...
          <div className="h-16 bg-[#F1F5F9] rounded-lg flex items-center justify-center mb-2">
        ...
        <div className="border-2 border-[#16A34A] rounded-xl p-3 relative">
          <span className="absolute -top-2.5 left-2 text-[9px] font-bold text-white bg-[#16A34A] px-2 py-0.5 rounded-full uppercase">🏆 Winner</span>
          <p className="text-[10px] font-semibold text-[#94A3B8] uppercase tracking-wider mb-2">Creative B</p>
          <div className="h-16 bg-[#F0FDF4] rounded-lg flex items-center justify-center mb-2">
            <svg className="w-6 h-6 text-[#86EFAC]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14" />
            </svg>
          </div>
          <div className="flex items-baseline gap-0.5">
            <span className="text-[22px] font-black text-[#16A34A]">89</span>
            <span className="text-[10px] text-[#94A3B8]">/100</span>
          </div>
        </div>
      </div>
    ...
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 md:p-5">
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ...
          <div key={s.label} className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg p-2 text-center">
            <p className="text-[18px] font-black text-[#0F172A]">{s.value}</p>
            <p className="text-[10px] text-[#64748B]">{s.label}</p>
          </div>
        ))}
      </div>
      <p className="text-[11px] font-semibold text-[#64748B] uppercase tracking-wider mb-2">Recent Analyses</p>
      <div className="space-y-1.5">
        {analyses.map((a) => (
          <div key={a.label} className="flex items-center gap-2 py-1.5 border-b border-[#F1F5F9] last:border-0">
            ...
            <span className="flex-1 text-[11px] text-[#334155] truncate">{a.label}</span>
            <span
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="text-center mb-8 md:mb-12 px-6">
        ...
        <h2 className="text-[26px] md:text-[36px] lg:text-[40px] font-bold text-[#0F172A] leading-tight tracking-tight">
          How performance teams go from<br className="hidden md:block" /> guessing to scaling.
        </h2>
      </div>

      {/* Tab bar — horizontal scroll on mobile, centered wrap on desktop */}
      <div
        className="flex gap-2 mb-6 md:mb-8 md:justify-center md:flex-wrap overflow-x-auto md:overflow-visible [&::-webkit-scrollbar]:hidden px-6 md:px-4"
      ...
      >
        {TABS.map((tab) => (
          <button
            ...
            className={`shrink-0 inline-flex items-center gap-2 px-4 py-2.5 rounded-[8px] text-[14px] font-medium border transition-all duration-150 whitespace-nowrap ${
              activeTab === tab.id
      ...
      {/* Browser mockup */}
      <div className="mx-4 md:mx-0 bg-white rounded-[12px] md:rounded-[16px] border border-[#E2E8F0] shadow-[0_4px_20px_rgba(0,0,0,0.06)] md:shadow-[0_8px_30px_rgba(0,0,0,0.08)] overflow-hidden">
        {/* Chrome bar */}
        <div className="bg-[#F8FAFC] border-b border-[#E2E8F0] px-3 md:px-4 py-2.5 md:py-3 flex items-center gap-3">
          ...
          <div className="flex-1 flex justify-center">
            <div className="bg-white border border-[#E2E8F0] rounded-md px-3 py-1 text-[11px] md:text-[12px] text-[#94A3B8] w-full max-w-[200px] md:max-w-[340px] text-center select-none truncate">
        ...
        {/* Content */}
        <div className="p-3 md:p-6 bg-[#F8FAFC] min-h-[260px] md:min-h-[380px]">
